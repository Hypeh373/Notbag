...
# --------------------------- ОСНОВНЫЕ НАСТРОЙКИ ----------------------------------
# =================================================================================
CREATOR_BOT_TOKEN = '8589657575:AAFdA9eO2XRAHa0GZ_jRmiR0HE2DKb1SN-E'  # 👈 Вставь свой токен конструктора
CRYPTO_PAY_TOKEN = '467156:AA0Mvgp0h5oKZaETFQZqdnCWUZSoPVpAT0W' # 👈 Получи его в @CryptoBot -> Crypto Pay -> Создать приложение

ADMIN_ID = 8395830207 # 👈 ТВОЙ ID АДМИНИНИСТРАТОРА
...
PROMO_BOT_SCRIPT_NAME = 'promo_market_bot.py'
GUARANT_BOT_SCRIPT_NAME = 'guarant_bot.py'
...
PROMO_GLOBAL_SETTING_KEY = 'promo_global_unlocked'
...
BUTTON_KEY_DESCRIPTIONS = {
    ...
    'create_guarant': 'Кнопка выбора типа "Гарант-бот"',
}
...
def init_db():
    with db_lock:
        ...
        if 'promo_unlocked' not in user_columns:
            cursor.execute("ALTER TABLE users ADD COLUMN promo_unlocked BOOLEAN DEFAULT FALSE")
            logging.info("Колонка 'promo_unlocked' добавлена в таблицу 'users'.")
        ...
        new_columns = {
            ...
            # Promo (Биржа рекламы) specific settings
            'promo_crypto_pay_token': "TEXT",
            'promo_listing_price': "TEXT",
            'promo_allow_free': "INTEGER DEFAULT 0",
            'promo_flyer_api_key': "TEXT",
            # Guarant (Гарант) specific settings
            'guarant_crypto_pay_token': "TEXT",
            'guarant_fee': "REAL DEFAULT 5.0",
            'guarant_min_deal_amount': "REAL DEFAULT 1.0",
            'guarant_flyer_api_key': "TEXT",
        }
...
def update_bot_setting(bot_id, setting_name, new_value):
    allowed_settings = [
        ...
        # Promo (Биржа рекламы) specific settings
        'promo_crypto_pay_token', 'promo_listing_price', 'promo_allow_free',
        # Guarant (Гарант) specific settings
        'guarant_crypto_pay_token', 'guarant_fee', 'guarant_min_deal_amount', 'guarant_flyer_api_key',
    ]
...
def get_bot_creation_button_texts():
    return {
        ...
        'guarant': get_custom_button_text('create_guarant'),
    }
...
def is_promo_unlocked_globally():
    try:
        value = get_setting(PROMO_GLOBAL_SETTING_KEY)
    except Exception:
        value = None
    if value is None:
        return False
...
def unlock_promo_globally():
    try:
        set_setting(PROMO_GLOBAL_SETTING_KEY, '1')
        db_execute(
            "UPDATE users SET promo_unlocked = 1 WHERE promo_unlocked IS NULL OR promo_unlocked = 0",
            commit=True
        )
    except Exception as exc:
        logging.error(f"Не удалось установить глобальный доступ к Promo Bot: {exc}")
        raise
...
def get_max_bots_limit_for_user(user_id):
    """
    Возвращает (can_create, limit, count) для проверки лимита бесплатных ботов.
    Админы не ограничены лимитом.
    """
    try:
        ...
        count = 0
    try:
        raw = get_setting('MAX_BOTS_PER_USER')
        limit = int(float(raw)) if raw is not None else int(MAX_BOTS_PER_USER)
    except Exception:
        limit = int(MAX_BOTS_PER_USER)
    if is_admin(user_id):
...
def start_bot_process(bot_id):
    ...
    try:
        ...
        elif bot_info['bot_type'] == 'promo':
            script_name = PROMO_BOT_SCRIPT_NAME
            env['PROMO_DB_PATH'] = os.path.abspath(f"dbs/bot_{bot_id}_promo.db")
            if bot_info['bot_token']:
                env['PROMO_BOT_TOKEN'] = bot_info['bot_token']
            promo_crypto_token = bot_info.get('promo_crypto_pay_token')
            if promo_crypto_token and promo_crypto_token.strip() and promo_crypto_token not in ('—', ''):
                env['PROMO_CRYPTO_PAY_TOKEN'] = promo_crypto_token.strip()
                logging.info(f"Promo bot {bot_id}: Crypto Pay token passed to env (length: {len(promo_crypto_token.strip())})")
            else:
                logging.warning(f"Promo bot {bot_id}: Crypto Pay token not found or empty")
            if bot_info.get('promo_listing_price'):
                env['PROMO_LISTING_PRICE'] = str(bot_info['promo_listing_price'])
            if bot_info.get('promo_allow_free'):
                env['PROMO_ALLOW_FREE'] = '1'
            if bot_info.get('promo_flyer_api_key'):
                env['PROMO_FLYER_API_KEY'] = bot_info['promo_flyer_api_key']
            env['PROMO_ADMIN_IDS'] = str(bot_info['owner_id'])
            link_raw = get_custom_text('constructor_bot_link')
            link_url = normalize_creator_link_value(link_raw)
            if link_url:
                env['CREATOR_USERNAME'] = link_url
            env['PROMO_HIDE_BRANDING'] = '1' if bot_info.get('vip_status') else '0'
        elif bot_info['bot_type'] == 'guarant':
            script_name = GUARANT_BOT_SCRIPT_NAME
            env['GUARANT_DB_PATH'] = os.path.abspath(f"dbs/bot_{bot_id}_guarant.db")
            if bot_info['bot_token']:
                env['GUARANT_BOT_TOKEN'] = bot_info['bot_token']
            guarant_crypto_token = bot_info.get('guarant_crypto_pay_token')
            if guarant_crypto_token and guarant_crypto_token.strip() and guarant_crypto_token not in ('—', ''):
                env['GUARANT_CRYPTO_PAY_TOKEN'] = guarant_crypto_token.strip()
                logging.info(f"Guarant bot {bot_id}: Crypto Pay token passed to env (length: {len(guarant_crypto_token.strip())})")
            else:
                logging.warning(f"Guarant bot {bot_id}: Crypto Pay token not found or empty")
            if bot_info.get('guarant_fee'):
                env['GUARANT_FEE'] = str(bot_info['guarant_fee'])
            if bot_info.get('guarant_min_deal_amount'):
                env['MIN_DEAL_AMOUNT'] = str(bot_info['guarant_min_deal_amount'])
            if bot_info.get('guarant_flyer_api_key'):
                env['GUARANT_FLYER_API_KEY'] = bot_info['guarant_flyer_api_key']
            env['GUARANT_ADMIN_IDS'] = str(bot_info['owner_id'])
            link_raw = get_custom_text('constructor_bot_link')
            link_url = normalize_creator_link_value(link_raw)
            if link_url:
                env['CREATOR_USERNAME'] = link_url
            env['GUARANT_HIDE_BRANDING'] = '1' if bot_info.get('vip_status') else '0'
        else:
...
def create_bot_type_menu(user_id=None):
    ...
    # Промо-маркет (биржа рекламы) - требует разблокировки кодом
    promo_available = is_promo_unlocked_globally()
    if not promo_available and user_id is not None:
        try:
            user = get_user(user_id)
            promo_available = bool(user.get('promo_unlocked', False)) if user else False
        except Exception:
            promo_available = False
    if promo_available:
        markup.add(types.InlineKeyboardButton(creation_buttons.get('promo', "📢 Биржа рекламы"), callback_data="create_bot_promo"))
    
    # Гарант-бот - доступен всем
    markup.add(types.InlineKeyboardButton(creation_buttons.get('guarant', "🛡️ Гарант-бот"), callback_data="create_bot_guarant"))
...
def create_promo_bot_config_menu(bot_id):
    bot_info = get_bot_by_id(bot_id)
    if not bot_info:
        return None
    markup = types.InlineKeyboardMarkup(row_width=2)
    ...
    markup.add(
        types.InlineKeyboardButton("🔑 Токен бота", callback_data=f"edit_{bot_id}_bot_token"),
        types.InlineKeyboardButton("💳 Crypto Pay токен", callback_data=f"edit_{bot_id}_promo_crypto_pay_token"),
    )
    markup.add(
        types.InlineKeyboardButton("💵 Цена размещения", callback_data=f"edit_{bot_id}_promo_listing_price"),
        types.InlineKeyboardButton("🆓 Бесплатный режим", callback_data=f"promo_{bot_id}_toggle_free"),
    )
    markup.add(
        types.InlineKeyboardButton("👥 Админы", callback_data=f"admins_{bot_id}_manage"),
    )
...
def create_guarant_bot_config_menu(bot_id):
    bot_info = get_bot_by_id(bot_id)
    if not bot_info:
        return None
    markup = types.InlineKeyboardMarkup(row_width=2)
    ...
    markup.add(
        types.InlineKeyboardButton("🔑 Токен бота", callback_data=f"edit_{bot_id}_bot_token"),
        types.InlineKeyboardButton("💳 Crypto Pay токен", callback_data=f"edit_{bot_id}_guarant_crypto_pay_token"),
    )
    markup.add(
        types.InlineKeyboardButton("💸 Комиссия гаранта", callback_data=f"edit_{bot_id}_guarant_fee"),
        types.InlineKeyboardButton("💰 Мин. сумма сделки", callback_data=f"edit_{bot_id}_guarant_min_deal_amount"),
    )
    markup.add(
        types.InlineKeyboardButton("👥 Админы", callback_data=f"admins_{bot_id}_manage"),
    )
...
def create_dop_zarabotok_menu(bot_id):
    ...
    if bot_info['bot_type'] == 'ref':
        ...
        is_enabled = bool(bot_info.get('guarant_flyer_api_key'))
        op_reward = get_setting('op_reward') or "1.0"
        text = (f"🛡️ *Гарант-бот + Flyer*\n\n"
                f"Подключите систему обязательной подписки (ОП) к вашему гарант-боту и получайте *{op_reward} ₽* за каждого уникального пользователя, который её пройдет!\n\n"
                f"Это отличный способ монетизировать аудиторию вашего бота.")
    else:
...
def process_state_input(message):
    ...
    if action == 'editing_setting':
        ...
        if new_value_raw is None:
            ...
            try:
                ...
                if bot_info['status'] == 'unconfigured': update_bot_setting(bot_id, 'status', 'stopped')
                         ...
                         'anonchat_vip_price', 'guarant_fee', 'guarant_min_deal_amount']:
            try:
            ...
            update_bot_setting(bot_id, setting, cleaned)
        elif setting in ['cashlait_flyer_api_key', 'cashlait_crypto_pay_token', 'cashlait_currency_symbol', 'dicelite_crypto_pay_token', 'dicelite_welcome_text', 'exchange_welcome_text', 'promo_crypto_pay_token', 'promo_listing_price', 'promo_flyer_api_key', 'guarant_crypto_pay_token', 'guarant_flyer_api_key']:
            cleaned = '' if new_value_raw.strip() == '-' else new_value_raw.strip()
            update_bot_setting(bot_id, setting, cleaned)
        ...
        if bot_info['bot_type'] == 'ref':
            ...
            config_menu = create_promo_bot_config_menu(bot_id)
        elif bot_info['bot_type'] == 'guarant':
            config_menu = create_guarant_bot_config_menu(bot_id)
...
if __name__ == '__main__':
    ...
    def handle_text_buttons(message):
        ...
        # Secret code to unlock 'Биржа рекламы' (Promo) bot type for this user
        if text_value == PROMO_UNLOCK_CODE:
            already_global = is_promo_unlocked_globally()
            try:
                db_execute("UPDATE users SET promo_unlocked = 1 WHERE user_id = ?", (user_id,), commit=True)
            except Exception as e:
                logging.error(f"Не удалось установить флаг promo_unlocked для пользователя {user_id}: {e}")
            if not already_global:
                try:
                    unlock_promo_globally()
                    logging.info(f"Пользователь {user_id} открыл доступ к типу 'Биржа рекламы' для всех пользователей")
                except Exception as e:
                    logging.error(f"Не удалось открыть доступ к Биржа рекламы глобально: {e}")
            confirmation_text = (
                "✅ Тип бота 'Биржа рекламы' уже был доступен всем пользователям!"
                if already_global
                else "✅ Новый тип бота 'Биржа рекламы' теперь доступен всем пользователям!"
            )
            bot.send_message(user_id, confirmation_text, parse_mode="HTML")
            bot.send_message(user_id, get_custom_text('create_bot_prompt'), parse_mode="HTML", reply_markup=create_bot_type_menu(user_id))
            return
        ...
        if message.text == main_buttons['create']:
            can_create, limit_setting, count = get_max_bots_limit_for_user(user_id)
            if not can_create:
    ...
    def handle_callback_query(call):
        ...
        try:
            ...
            # Обработка кнопок личного кабинета (вывод и история)
            ...
            if call.data.startswith('flyer_op_'):
                ...
                if action == 'apply':
                    ...
                    if action == 'approve':
                        ...
                        msg = bot.edit_message_text(admin_text + "\n\n<b>Статус: ОЖИДАНИЕ КЛЮЧА</b>", call.message.chat.id, call.message.message_id, parse_mode="HTML")
                        set_user_state(user_id, {
            ...
            # Дополнительная защита лимита при создании бота через inline-кнопки
            if call.data in ("create_bot_ref", "create_bot_stars", "create_bot_clicker", "create_bot_anonchat", "create_bot_promo", "create_bot_guarant"):
                can_create, limit_setting, count = get_max_bots_limit_for_user(user_id)
                if not can_create:
                    bot.answer_callback_query(
                        call.id,
                        f"❌ Лимит достигнут! Вы уже создали {count} из {limit_setting} ботов.",
                        show_alert=True
                    )
                    return
            ...
            if call.data == "create_bot_promo":
                bot.answer_callback_query(call.id, "Бот создается...")
                bot_id = create_bot_in_db(user_id, 'promo')
                bot.edit_message_text(f"📢 Бот 'Биржа рекламы' #{bot_id} создан! Теперь он в списке ваших ботов:", user_id, call.message.message_id, reply_markup=create_my_bots_menu(user_id)); return
            if call.data == "create_bot_guarant":
                bot.answer_callback_query(call.id, "Бот создается...")
                bot_id = create_bot_in_db(user_id, 'guarant')
                bot.edit_message_text(f"🛡️ Бот 'Гарант' #{bot_id} создан! Теперь он в списке ваших ботов:", user_id, call.message.message_id, reply_markup=create_my_bots_menu(user_id)); return
            
            # Обработчик переключения бесплатного режима для промо-бота
            if call.data.startswith("promo_") and "_toggle_free" in call.data:
                parts = call.data.split("_")
                if len(parts) >= 3:
                    bot_id = int(parts[1])
                    bot_info = get_bot_by_id(bot_id)
                    if bot_info and bot_info['owner_id'] == user_id:
                        current_free = bot_info.get('promo_allow_free', 0)
                        new_value = 0 if current_free else 1
                        db_execute("UPDATE bots SET promo_allow_free = ? WHERE id = ?", (new_value, bot_id), commit=True)
                        status = "✅ Включён" if new_value else "❌ Выключен"
                        bot.answer_callback_query(call.id, f"Бесплатный режим: {status}", show_alert=True)
                        config_menu = create_promo_bot_config_menu(bot_id)
                        name = bot_info['bot_username'] or f"#{bot_id}"
                        try:
                        ...
                        bot.answer_callback_query(call.id, "❌ Бот не найден или вы не владелец.", show_alert=True)
                return
            ...
            if action == 'actions':
                ...
                prompts = {
                    ...
                    'promo_crypto_pay_token': "💳 Введите Crypto Pay токен (из @CryptoBot) для Биржи рекламы:",
                    'promo_listing_price': "💵 Введите цену размещения (в $):",
                    'promo_flyer_api_key': "🪁 Введите Flyer API ключ для ОП (обязательной подписки):",
                    'guarant_crypto_pay_token': "💳 Введите Crypto Pay токен (из @CryptoBot) для Гарант-бота:",
                    'guarant_fee': "💸 Введите комиссию гаранта (в %, например 5.0):",
                    'guarant_min_deal_amount': "💰 Введите минимальную сумму сделки (в USDT, например 1.0):",
                }
